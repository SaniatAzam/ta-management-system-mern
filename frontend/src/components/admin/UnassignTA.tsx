...
import UnassignTARow from "./UnassignTARow";
  ...
  const fetchCourseData = async () => {
    try {
      ...
      for (const d of data.courses) {
        ...
        // for (const tas of d.assignedTAs)
        //   courseAssignedTARes.add(await fetch("http://127.0.0.1:3000/api/users/" + tas));
        let item = {};
        if (courseInstructorRes) {
          ...
          item["term"] = d.term;
          item["year"] = d.year;
          item["assignedTAs"] = numOfAssignedTAs + " / " + d.quota;
          ...
          <table>
            ...
            <tbody>
              ...
              {Courses.map((course: Course, i: number) => {
                if (course) {
                  return <UnassignTARow key={i} course={course} fetchCourseData={fetchCourseData} />;
                }
                return null;
              })}
            </tbody>
